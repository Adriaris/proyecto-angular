...
@Component({
  selector: 'app-perfilcrud',
  templateUrl: './perfilcrud.component.html',
  styleUrls: ['./perfilcrud.component.css'],
...
export class PerfilcrudComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Hacer una petición HTTP al servidor para obtener los datos del usuario correspondiente al token
  this._userService.getUserByToken().subscribe(
    response => {
      if (response) {
        ...
        this.selectedCharacters.push(this.user.id_first_character ?? 0);
        this.selectedCharacters.push(this.user.id_second_character ?? 0);
        this.selectedCharacters.push(this.user.id_third_character ?? 0);
